...
- 平均互动率（avg_interaction_rate，保留 3 位小数）
- 图片占比（image_ratio，保留 1 位小数，加%符号）
- 视频占比（video_ratio，保留 1 位小数，加%符号）
- 平均点赞数（avg_likes，保留整数）
- 内容数量（post_count）

## 提示

- 使用 SUM 和 AVG 函数进行聚合计算
- 使用 CASE WHEN 计算占比
- 使用 HAVING 过滤分组结果
- 注意数据类型转换
