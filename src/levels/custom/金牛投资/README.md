...
投资总监想要了解各行业板块的交易情况，请你编写 SQL 查询：
...
- 行业板块（sector）
- 总交易金额（total_amount，保留 2 位小数）
- 交易笔数（transaction_count）
- 平均股价（avg_price，保留 2 位小数）
- 买入占比（buy_ratio，保留 1 位小数，加%符号）
- 机构占比（institution_ratio，保留 1 位小数，加%符号）
- 平均手续费（avg_commission，保留 2 位小数）
- 热门股票（top_stock）
- 价格波动率（volatility，保留 1 位小数，加%符号）
