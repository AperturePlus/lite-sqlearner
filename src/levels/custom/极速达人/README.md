...
- `work_hours`：工作时长（小时）
- `customer_rating`：客户评分（1-5 分）
- `delivery_area`：配送区域

## 任务要求

运营总监想要一个综合的快递员绩效报告，请你编写 SQL 查询：
...
- 总失败订单（total_failed）
- 成功率（success_rate，保留 1 位小数，加%符号）
- 平均评分（avg_rating，保留 1 位小数）
- 总配送距离（total_distance，保留 1 位小数）
- 工作效率（efficiency，保留 1 位小数，单位：单/小时）
